...
def Initial_population_initialization(data, desiredSequenceSize):
    ...
    for a in data:
        ...
        numList = func_numlist(desiredSequenceSize)
...
def func_numlist(desiredSequenceSize):
...
def Parent_gen(data, desiredSequenceSize):
    '''
    from the original sequences creats two parents
    '''
    parents = [Initial_population_initialization(data, desiredSequenceSize), Initial_population_initialization(data, desiredSequenceSize)]

    return parents


def OperatorSinglePoint(data, randpos, originalSequences):
    '''
...
def OperatorUniformBlockExchange(data, originalSequences):
    g1 = None
    g2 = None
...
def OperatorRecombineMatchColumn(data, numOfSequences):
    father = data[0]
    mother = data[1]
...
def OperatorChnageSpace(data, lamda):
    '''
...
def OperatorMergeSpaceBLock(data, lamda):
    ''''
...
def OperatorCollectSpace(data, lamda):
    '''
...
def OperatorMoveToHigherScore(data, lamda):
    copi = data.copy()
...
def CroOperatorDecomposition(data, lamda):
    #    print ("Decomposition")
    ...
    #    print (work)
    child1 = OperatorChnageSpace(work, lamda)
    child2 = OperatorMoveToHigherScore(work, lamda)
...
##########################>?MAIN?<###################################
def genetic_main(dim, q):

    outputfile = open("result/genetic.txt", "w+")
    ...
    for i in range(dim):
        #originalSequences.append(q[i])
        originallen.append(len(q[i]))
    ...
    for a in range(iteration):
        ...
        '''GA Start'''
        # Sexual reproduction
        parents = Parent_gen(originalSequences, desiredSequenceSize)
        parents = OperatorSinglePoint(parents, randpos, originalSequences)
        #    parents = OperatorUniformBlockExchange(parents)

        # Mutation
        offspiring = OperatorRecombineMatchColumn(parents, numOfSequences)
        offspiring = OperatorChnageSpace(offspiring, lamda)
        offspiring = OperatorDecreaseSearchSpace(offspiring)
        offspiring = OperatorMergeSpaceBLock(offspiring, lamda)
        offspiring = OperatorColumnAllDash(offspiring)
        offspiring = OperatorCatchMatchColumn(offspiring)
        offspiring = OperatorCollectSpace(offspiring, lamda)
        #offspiring = OperatorMoveToHigherScore(offspiring, lamda)
        ...
        #parents = CroOperatorDecomposition(offspiring, lamda)
